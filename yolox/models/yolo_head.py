...
class YOLOXHead(nn.Module):
    ...
    def get_output_and_grid(self, output, k, stride, dtype):
        ...
        n_ch = 9 + self.num_classes  # 8 coords + 1 obj + num_classes
        ...
        # Decode polygon vertices: each vertex is relative to the grid cell
        # Vertices 0-3: (x1,y1), (x2,y2), (x3,y3), (x4,y4)
        for i in range(4):
    ...
    def decode_outputs(self, outputs, dtype):
        ...
        # Decode polygon vertices
        decoded_coords = []
        for i in range(4):
            vertex = (outputs[..., i*2:i*2+2] + grids) * strides
            decoded_coords.append(vertex)
        
        outputs = torch.cat(
            decoded_coords + [outputs[..., 8:]],
            dim=-1
        )
        return outputs
    ...
    def get_l1_target(self, l1_target, gt, stride, x_shifts, y_shifts, eps=1e-8):
        # For polygons, compute L1 target for each vertex relative to grid cell
        for i in range(4):
    ...
    def get_assignments(
        ...
        # Use polygon IoU for polygon bounding boxes
        pair_wise_ious = polygon_iou(gt_bboxes_per_image, bboxes_preds_per_image, use_torch=True)
